//run with root --hsfit PolSphHarmonicMoments0.C
{
  ...
  Flat.SetUp().LoadCategory("PolState[p1=1,p2=2]");

  
  Flat.SetUp().LoadFunctionVar("RooConstVar::Val(1)");
  Flat.SetUp().FactoryPDF("RooComponentsPDF::Flat(1,{CosTh,Phi,PolPhi,PolState},=Val)");
  ...
  Generator.SetUp().LoadCategory("PolState[p1=1,p2=2]");
  
  Generator.SetUp().LoadParameter("Pol[0.9]");
  
  Generator.SetUp().LoadFormula("PolPlane=(@PolState[]==1)*0./2+(@PolState[]==2)*3.14159/2");
  ...
  auto configGenPDF=HS::FIT::EXPAND::ComponentsPolSphHarmonicPolStateFixPol(Generator.SetUp(),"Moments","CosTh","Phi","PolPhi","PolState","Pol","PolPlane",3,2,kTRUE);
  ...
  Generator.SetUp().WS().var("H1_0_0")->setVal(-0.3);
  ...
  Fitter.SetUp().LoadCategory("PolState[p1=1,p2=2]");

    //Fitter.SetUp().LoadVariable("Pol[0.5,0.2,0.6]");
  Fitter.SetUp().LoadParameter("Pol[0.9]");
  Fitter.SetUp().LoadFormula("PolPlane=(@PolState[]==1)*(0.)+(@PolState[]==2)*(3.141/2)");

  auto configFitPDF=HS::FIT::EXPAND::ComponentsPolSphHarmonicPolStateFixPol(Fitter.SetUp(),"Moments","CosTh","Phi","PolPhi","PolState","Pol","PolPlane",3,2,kTRUE);
