...
void LoadExperiment(){
  ...
  if(C12TOOL!=TString("")) {
    ...
    gROOT->SetMacroPath(Form("%s:%s/Hipo4",gROOT->GetMacroPath(),C12TOOL.Data()));
    gSystem->AddIncludePath(TString("-I")+C12TOOL+"/Hipo4");
	...
	gROOT->LoadMacro(C12TOOL+"/Hipo4/"+name+".cpp+");
    }
    
    std::vector<TString > C12BClasses={"header","particle","mcparticle","particle_detector","scintillator","calorimeter","cherenkov","forwardtagger","tracker","covmatrix","scaler","vtp","region_particle","region_ft","region_fdet","region_cdet","clas12reader"};
